...
| `design-brief-builder` | 设计、设计规范、设计 brief、UI、UX、交互、布局、信息架构、中文优先、人类可读、人话文案、程序代码信息、机器字段、技术字段外露、工具地图、一级页面、页面职责、用户路径、功能定位、视觉方向、界面风格、设计参考、页面状态、空状态、产品气质、视觉规范、响应式、移动端、桌面端、可访问性、文案语气、颜色、字体、圆角、密度、Figma 审查、设计审查、首屏任务、行动路径、审计信息折叠、决策卡片、候选卡片、复盘卡片、报告卡片、行动卡片、核心业务判断、决策意义、验证工作台、内部模拟执行、聊聊界面、讨论交互、页面怎么组织、这个界面怎么设计、用户怎么操作 |
...
| `review-runner` | 独立审查、总体验收、闭环审查、流程审计、检查是否完成、完成了吗、验收证据、审查报告、第二视角、新脑子、证据够不够、有没有闭环缺陷、是否可以收工、是否可以发布、GOAL 完成检查、用户闭环是否推进、持续交付是否被流程拖慢、是不是又在证明局部零件、前端是否人类可读、前端中文优先、前端技术字段外露、前后端是否对齐、联调是否完成、前端后端一致性、真实接入是否完成、模型/API 是否真的跑通 |
...
| `evolution-runner` | 进化、自进化、自净化、规则更新、沉淀规则、signals、失败信号、复盘规则、删规则、规则膨胀、问题库、调优框架、优化其他 Skill、让 Skill 更适应项目、自动进化本地 Skills、净化规则、净化 schema、调整 Skill 边界、机制瘦身、Value Slice、Product Value Gate、consumer 泛滥、handoff 泛滥、日志分流、审查去抖、中文文档规则、文档语言规则、所有 Skill 文档中文 |
...
| `frontend-quality-runner` | 前端体验质量、UI 质感、信息架构、页面可读性、中文优先、人类可读、人话文案、程序代码信息、机器字段、技术字段外露、文本溢出、浏览器截图验收、Figma 审查、Figma 插件、设计审查、首屏没有重点、行动路径不清、审计信息太多、像日志罗列、dashboard 好不好用、工作台好不好用、工具地图、一级页面、页面职责、移动端适配、决策卡片、候选卡片、复盘卡片、报告卡片、行动卡片、核心业务判断、当前判断、决策意义、机器字段卡片、验证工作台、结果工作台、内部模拟执行、结果曲线 |
| `open-source-research-runner` | GitHub 调研、开源库选型、找资源、找实现、github-research、provider 候选、验证引擎、图表库、数据处理库、license 评估、嫁接别人开发好的前后端、开源前端、开源后端、Open Source Integration Gate |
...
| 探讨技术方案、架构、数据源、开源库、provider/API、模型或能力接入 | `project-orchestrator` -> `dev-planner`; 涉及真实调用/secret/条款/成本时加 `gate-runner` 或 `code-reviewer` | `planning`, `guardian`; 确认后才到 `development` | `docs/09-research-roadmap.md`, `docs/capability-status.json`, provider card | 低风险调研可记录和派规划；真实 provider/API/模型/付费/账号/受监管操作必须先进 `guardian` |
| 探讨 UI、页面体验、信息架构、交互状态、面板或原型 | `design-brief-builder`; 需要 demo 时加 `prototype-builder`；涉及复杂工作台或截图问题时加 `frontend-quality-runner` 的 Figma 审查清单 | `design` | `docs/02-design-brief.md`, 原型或 artifact 路径 | 有明确页面、用户操作或状态覆盖要求时派发；未确认产品方向时只写设计影响 |
| 讨论实现、联调、本地验证、脚本、fixture、端到端样本 | `dev-builder`, `bug-fixer`, `gate-runner` | `development` | `scripts/`, `artifacts/`, `src/`, `docs/03-dev-plan.md`, `docs/capability-status.json` | 需求/边界已清楚且无外部授权风险时派发；否则先 `planning` 或 `guardian` |
...
| 讨论边界写死、不要把边界写成禁止、授权后还被挡住、真实接入被机制拦住、quality_reviewed / production_ready 永远上不去、provider/live API、secret、scheduler、writeback、production feed 或高风险外部执行被长期禁止 | `project-orchestrator` -> `evolution-runner`; 涉及具体授权执行时加 `gate-runner` 或 `guardian` | `evolution` 或 `guardian` | `AGENTS.md`, `.agents/skills/project-orchestrator/SKILL.md`, `.agents/skills/dev-builder/SKILL.md`, `.agents/skills/review-runner/SKILL.md`, `.agents/skills/gate-runner/SKILL.md`, `.codex/gates/skill-mechanism-check.ps1` | 触发 Dynamic Boundary Gate：把死边界改成授权闸门；已有授权时进入受控开发/验证，不重复确认；未授权时给最短授权路径 |
...
| 讨论前端质感、页面可读性、中文优先、人类可读、技术字段外露、信息架构、Figma 审查或浏览器截图验收 | `frontend-quality-runner` | `design`, `development` 或 `review` | `docs/02-design-brief.md`, `docs/05-review-report.md`, `artifacts/<slice>/` | 用户可见 UI、dashboard、研究工作台或截图证据不足时派发；必须检查首屏任务、行动路径、主次分层、审计折叠、状态语义、人话文案、Human-Readable Frontend Gate、窄屏可用和风险提示 |
| 讨论研究卡片、候选卡片、复盘卡片、报告卡片、行动卡片、买入判断或卡片是否有决策意义 | `design-brief-builder` + `frontend-quality-runner` | `design` 或 `review` | `docs/02-design-brief.md`, `docs/05-review-report.md`, `artifacts/<slice>/` | 必须执行 Decision-Usefulness Gate：声明卡片类型，回答当前判断、买入判断影响、关键证据、否决/风险条件、下一步人工动作，并把技术审计折叠；不能让机器字段卡片通过 |
| 讨论工具地图、一级页面、页面职责、用户路径、验证工作台、内部模拟执行或跨页面产品 IA | `design-brief-builder` + `frontend-quality-runner` | `design` 或 `review` | `docs/02-design-brief.md`, `docs/05-review-report.md`, `docs/frontend-tool-usage-map.md`, `artifacts/<slice>/` | 必须执行 Tool-Level IA Gate：从用户目标定义一级页面、页面职责、用户路径、入口、结果回看、状态边界和字段用途；验证实验/内部模拟执行不能只藏在报告或审计字段里 |
| 讨论内部模拟执行、沙盒操作、模拟结果、虚拟资源、结果曲线或预演工作流语义 | `design-brief-builder` + `frontend-quality-runner`; 涉及真实账号/真实资源/secret/scheduler 时加 `guardian` | `design`, `review` 或 `guardian` | `docs/02-design-brief.md`, `docs/05-review-report.md`, `docs/capability-status.json`, 泳道 workspace | 必须执行 Internal Simulation Semantics Gate：区分本地样本阶段、产品内受控模拟操作和高风险外部执行；不得把内部模拟执行等同真实外部执行，也不得降格成只读审计文本 |
| 讨论开源库、GitHub 资源、provider 候选、外部实现选型或嫁接别人开发好的前后端 | `open-source-research-runner`; 涉及引入代码/依赖时加 `gate-runner` 或 `guardian` | `planning` 或 `guardian` | `docs/08-open-source-reference-pool.md`, `docs/09-research-roadmap.md`, `docs/capability-status.json`, 泳道 workspace | 必须先走 Open Source Integration Gate：license / attribution / dependency / local smoke / adapter / guardian 边界；不得直接复制外部前后端 |
...
- `confirmation_needed`: 涉及路线锁定、真实外部调用、付费、secret、账号、受监管操作、高风险真实执行、持久写入、生产声明或重大架构取舍，先给用户确认卡。
...
当用户讨论“前端底座、前端立项、复杂业务工作台、对象分析前端、图表/事件/表格/指标组件、L0/L1/L2/L3/L4 数据层、Design Token、目录结构、模块边界、组件复用、规划端和设计端共用前端流程”时，优先使用 `frontend-workflow-planner`。
...
- 如果涉及新依赖、复制外部项目源码、license/source attribution、provider live call、secret、真实账号、scheduler、真实远程资源或 production feed，先走 `guardian`。
