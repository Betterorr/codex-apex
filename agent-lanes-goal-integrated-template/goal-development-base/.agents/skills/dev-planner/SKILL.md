...
当主调度触发 Skeleton Plan Refresh 时，`dev-planner` 不只是重写阶段列表，而是维护 Money Digger 的产品骨架路线：
...
- `Production Pass`: 做批量、回滚、权限、日志、发布、成本、隐私和真实交易/外部写入边界。
...
- `90%+`: Production/Paper Pass，只能进入打包、长跑模拟盘、发布准备或实盘接入评估；不得跳过模拟盘直接实盘。
