...
Skeleton Plan Refresh 至少扫描 目标项目 产品骨架：
...
派发前先做阶段价值判断。不要把“能做的小闭环”误判成“当前最该做的主线”。模板部署到目标项目后，必须在计划文档里定义 P1-P5：

- `P1`: 当前用户必须走通的主线闭环。
- `P2`: 支撑主线的页面骨架、交互和信息架构。
- `P3`: 影响主线可信度的质量、真实样本、source/evidence 和稳定性问题。
- `P4`: 备注、导出、保存、偏好设置、二级筛选等辅助能力，默认 backlog。
- `P5`: 远程写入、真实账号动作、付费调用、scheduler、自动执行和生产承诺，默认守门/授权队列。
...
从 2026-07-01 起，主调度优先维护一个明确的 `active_user_loop`，默认是：

```text
<primary-user-entry> -> <object/detail-view> -> <core-action-or-analysis> -> <review-or-approval-flow> -> <report-or-evidence-output>
```
...
- planning / design / development / guardian / evolution callback 必须带 `active_user_loop`、`loop_impact`、`blocking_concerns`、`backlog_concerns`、`recommended_next_type`。
- review callback 必须带 `active_user_loop`、`user_loop_progress`、`blocking_concerns`、`backlog_concerns`、`recommended_next_type`。
...
2. 如果没有阻塞，立即运行 `Next Mainline Slice Selection`，从 `docs/03-dev-plan.md`、`docs/04-goal-log.md`、`agent-lanes/dashboard.md`、最新 callback 和 capability registry 中选择下一条 P1/P2 薄纵向切片。
...
4. 如果下一条 P1/P2 清楚且低风险，直接派发 planning / design / development / review 中最合适的泳道。
5. 如果下一条 P1/P2 不清楚，派 planning 做 `Skeleton Plan Refresh` 或 `Stage Value Refresh`，要求列出 3-6 个候选主线切片并推荐第一刀。
6. 阶段发布后最终状态不能只是 `idle`。必须已经派出下一条主线任务，或给出 `NEEDS_CONTEXT`，且原因只能属于下面三类。
...
- 是否需要用户授权、预算、账号、secret、真实写入、高风险自动化执行、路线锁定或产品取舍。
...
- 如果下一刀低风险、属于 P1/P2 或阻塞 P1/P2 的 P3，自动派对应泳道落实，不等待用户再次下令。
- 如果开发/设计/守门完成后仍缺验收证据，自动派 `review` 或 `gate-runner` 做聚焦核对。
- 如果发现风险边界不清，自动派 `guardian`；如果涉及必须用户决策的事项，停止并给用户确认卡。
- 如果当前阶段完成度达到 `80-90%`，自动进入 Quality Pass；达到 `90%+` 后，只能进入打包、长跑验证、发布准备或生产接入评估，不得直接进入高风险真实执行。
...
- `90%+`: Production/Paper Pass，做打包、长时间仿真验证、发布准备、权限/回滚/日志和生产接入评估。
...
- `confirmation_needed`: 涉及路线锁定、产品取舍、真实外部调用、付费、secret、账号、持久写入、受监管操作/高风险自动化执行、生产声明或重型框架采用。主调度先给用户确认卡，不派发产品化开发。
...
- 运行新的真实外部 provider/API、付费调用、secret、账号、受监管操作、高风险自动化执行或持久写入。
- 正式选择评估引擎、仿真验证机制、数据源策略、前端工程栈或长期运行架构。
