...
description: 当用户提到下一步、继续、接着做、往下推进、自动推进、轻量继续、高速产品工程经理、综合调用、总调度、根据当前项目状态决定做什么、继续闭环、推进项目、从当前状态开始、按 GOAL 流程继续、接手继续、你自己判断下一步、自动往后走、别问我先看项目、需求讨论、路线讨论、技术路线、我有个想法、我觉得、我们讨论一下、持续迭代、自动落档、脱手、自主迭代、自动规划、自动落实、callback hook、前端后端怎么推进、前后端联调、前端后端分流、分层推进、多泳道协作、并行泳道、泳道派发、泳道排队、任务排队、回复截断、上下文覆盖、任务被覆盖、泳道被打断、callback 打断主调度、主调度被打断、主调度收件箱、callback inbox、真实接入、外部 API 接入、模型接入、Provider 接入、能力状态 registry、单能力聚焦预算、capability exit check、provider 深挖收敛、Sketch Plan Loop、骨架计划循环、骨架优先、完成度、太慢、整体推进、项目还差什么、全链路扫描、中文文档、文档中文、项目文档中文优先时，使用这个 Skill 读取当前项目状态并选择合适的本地 Skill 工作流推进任务。
---
...
派发泳道任务时必须带上 `language_rule: 项目文档中文优先`，并要求目标泳道的 `summary`、`concerns`、`next_recommended_action` 和文档正文尽量中文。
...
Skeleton Plan Refresh 至少扫描通用产品骨架：
...
- 如果下一刀涉及真实外部调用、付费、secret、账号、交易、生产声明或重大路线取舍，先给用户确认卡。
...
主调度派发泳道任务前，必须先检查目标泳道是否已有未完成任务。Codex 线程不是可靠的多消息队列；同一个目标泳道不能同时收到多个 `active_dispatch`，否则后发消息可能打断当前执行，造成回复截断、上下文覆盖或任务堆积。

派发前检查：

- 从 `agent-lanes/message-log.jsonl`、`agent-lanes/dashboard.md` 和目标泳道 `worklog.md` 查找最新派发与 callback。
- 若目标泳道存在最新 `status` 为 `DISPATCHED`、`sent` 或 `active_dispatch` 的任务，且没有匹配 `reply_to` 的 completion callback，则该泳道视为 busy。
- busy 时不得继续对同一泳道调用 `send_message_to_thread`。把新任务写成 `queued_dispatch`；如果只是给当前任务补上下文，写成 `context_patch`，并用 `blocked_by_active_dispatch` 指向正在执行的 `message_id`。
- 等目标泳道 callback 返回后，主调度再合并同一泳道的 `queued_dispatch` 和 `context_patch`，发送一条完整的新派发。不要把多个后续指令拆成连续线程消息。
- 并行只允许发生在不同且不 busy 的泳道之间；同一泳道始终串行。
...
这条规则优先级高于 Autonomous Iteration Check 和普通并行泳道优化。自动调度发现下一步仍属于 busy 泳道时，只能排队或合并上下文，不能直接打断。

## Post Office Direct Callback / 邮局直投回报

`project-orchestrator` 不能把主调度线程当成可被任意泳道随时打断的碎片收件箱。非主调度泳道完成后必须通过 `agent-lanes/scripts/deliver_callback.py` 进入邮局，由邮局生成一条完整的 `thread_prompt`。

主调度的 callback 处理规则：

- 主调度收到 `【邮局合并回报】` 时，默认直接基于该消息原文处理，不要先回复“我去 message-log 读取完整 callback”。
- `message-log.jsonl` 只是审计备份；只有证据冲突、疑似重复、门禁报错、用户要求追溯或需要核对 raw JSON 时才打开。
- 邮局合并回报必须完整展开 `summary`、`changed_files`、`evidence`、`concerns`、`next_recommended_action` 等字段；不得用短 wake 或“详情请看收件箱”替代正文。
- 主调度处理同一批回报时先按 `message_id` 去重；没有稳定 `message_id` 时，按 `reply_to + from_agent + task_type` 折叠。
- 同一轮处理先形成一个 callback batch，再统一判断：继续派发、排队、计划刷新、守门、验收、问用户或收束。
- 处理 batch 期间不得因为新的 callback 改写当前决策；新 callback 进入下一轮 batch。
- 非主调度泳道不得绕过邮局向主调度发送短 wake。`deliver_callback.py` 返回 `send_required=false`、`spooled_waiting`、`spooled` 或 `batched_log` 时，说明回报尚未真正送达；泳道必须重跑投递或记录阻塞。

主调度输出或 worklog 应标记本轮处理到哪个 callback batch，例如 `batch_id`、`callback_ids` 或 `last_processed_callback_id`，避免下次重复消费同一批结果。
