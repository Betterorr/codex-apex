...
description: 当用户提到需求追踪、需求到设计/代码/测试映射、traceability matrix、需求覆盖、验收覆盖、需求漂移、阶段验收前检查、发布前需求闭环、哪个需求有没有实现或测试时，使用这个 Skill 在 MoneyDigger 现有 GOAL/Agent Lanes 文档体系内建立或审查需求追踪矩阵；输出必须落到 docs/05-review-report.md 或对应泳道 workspace，不得另起独立需求文档体系。
---

# MoneyDigger 需求追踪执行器

目标：吸收 `requirements-traceability` 的方法，但适配 MoneyDigger 的固定文档流，确保需求、设计、实现、测试和证据能互相追踪。
...
如涉及 provider、真实数据、外部 API、模型、券商或交易，还要读取：
...
- 对 fixture-only 能力，必须明确不能证明真实 provider、真实行情、真实交易或投资建议。
- 对外部 API、secret、付费、券商、交易和 scheduler，必须转守门泳道确认授权边界。
