...
description: 当用户提到需求追踪、需求到设计/代码/测试映射、traceability matrix、需求覆盖、验收覆盖、需求漂移、阶段验收前检查、发布前需求闭环、哪个需求有没有实现或测试时，使用这个 Skill 在 目标项目 现有 GOAL/Agent Lanes 文档体系内建立或审查需求追踪矩阵；输出必须落到 docs/05-review-report.md 或对应泳道 workspace，不得另起独立需求文档体系。
---

# 目标项目 需求追踪执行器

目标：吸收 `requirements-traceability` 的方法，但适配 目标项目的固定文档流，确保需求、设计、实现、测试和证据能互相追踪。
...
如涉及 provider、真实数据、外部 API、模型、受监管操作或高风险自动化执行，还要读取：
...
- 单泳道中间产物：写入 `agent-lanes/lanes/<lane>/workspace/traceability-<message_id>.md`
...
| --- | --- | --- | --- | --- | --- | --- |
| REQ-001 | ... | docs/02-design-brief.md | moneydigger/... | 命令/脚本 | artifact/docs 路径 | PASS/缺口/待确认 |
...
- 每条 in-scope 需求至少要映射到一个设计/实现/验证证据；缺任一关键环节就列为缺口。
- 对用户可见能力，必须确认入口、输入、输出、失败状态、复跑命令和可查看结果位置。
- 对 fixture-only 能力，必须明确不能证明真实 provider、真实线上数据、高风险真实执行或高风险结论。
- 对外部 API、secret、付费、受监管操作、高风险自动化执行和 scheduler，必须转守门泳道确认授权边界。
