...
- 当前 GOAL。
- 当前派发内嵌的 `value_slice` 与 Product Value Gate PASS 证据。
- `agent-lanes/current-state.json`
- `docs/product-feature-status.json`，当任务涉及用户页面、用户动作、workflow 或报告面。
...
- 编辑前先理解现有项目模式。
- 多泳道任务如果缺少 `value_slice_id`、`new_user_action`、`primary_surface` 或 Product Value Gate PASS 证据，不进入实现；回报主调度把任务保留为 draft/backlog。旧任务重放也不能绕过。
- 泳道的 `recommended_next_*` 只作咨询，不得由开发泳道自行触发 review、planning 或下一段开发。
- 改动范围贴合 GOAL，不做无关重构。
- 优先复用既有代码、设计资产和原型代码。
...
- 执行 Anti-Shell Gate：开发接到 handoff、readiness、browser smoke、screenshot、evidence-only、review surface、status panel 或 summary wrapper 类任务时，必须先确认派发记录里有 `stage_priority=P1/P2`、`mainline_impact=direct_advance` 或具体 `blocking_concerns`。如果没有，就不要继续加壳；把它改写为最小用户可见纵向接线，或回报主调度需要 `Next Mainline Slice Selection`。
...
- 用户页面、用户动作、workflow、报告面和 UI consumer 只更新 `docs/product-feature-status.json`；不得为 consumer、handoff、checkpoint、readiness、report mirror 或 review surface 新建 capability。仅增加页面镜像不能推进 `integration_connected`。
...
- `quality_reviewed`、`production_ready`、provider/live API、secret、scheduler、writeback、broker/trading、production feed、真实订单、真实资金或真实持仓是动态授权能力，不是永久禁止项。开发输出可以说明“本轮未推进”，但不能把它写成以后不得推进；如果任务上下文已有用户授权或 standing authorization，必须按授权范围进入受控安装、dry-run、metadata 检查、真实 smoke、样本保存、质量复核、接线或生产准备。
- 用户暂未授权 live call 时，只能完成合同、readiness、适配层或待执行队列，并把真实调用状态标成 `pending_user_approval`；不能用 fake/check 冒充已接入。用户已经授权时，不要继续用 `pending_user_approval` 拖住同范围任务。
...
- `agent-lanes/current-state.json`，只在 active Value Slice、blocking concern、backlog 或阶段状态改变时更新；不要把每条执行流水追加到当前状态。
...
如果本轮作为 development 泳道 completion callback 回报，JSON 必须额外包含：

- `message_id`、`reply_to`、`from_agent=开发泳道`、`to_agent=主调度泳道`：构成不可缺失的泳道身份与派发追踪链；不得依赖线程来源推断。
- `value_slice_id`: 当前 Value Slice。
- `value_delta`: 本轮新增、恢复或验证的用户价值。
- `review_policy`: 沿用派发中的合并验收策略。
- `active_user_loop`: 本轮服务的用户闭环。
- `loop_impact`: `advanced`、`blocked`、`regression_fixed` 或 `neutral`；普通开发任务不应是 `neutral`。
- `blocking_concerns`: 真正阻塞当前用户闭环的事项。
...
涉及 Value Delta completion 时，`fresh_at` 与 `completed_at` 必须使用命令真实执行时间并包含时区；不得填写未来时间、占位时间或为了通过门禁机械复制一个尚未发生的时间。completion 的 `source_message_id` 必须匹配 dispatch artifact 的 `outbound_message_id`，未提供时才匹配 `message_id`。

每条 completion evidence 必须有唯一 `evidence_id` 和 `receipt_path`；命令通过 `agent-lanes/scripts/evidence_receipt.py command ... -- <command>` 执行，receipt 必须记录真实 exit code 与 stdout/stderr hash；artifact 通过 `evidence_receipt.py artifact` 记录 hash/mtime。`acceptance_results[].evidence` 只能引用这些 `evidence_id`。不得手写 receipt、复用早于 dispatch 的旧 receipt，或只凭 `status=PASS` 自报新鲜证据。

V2.3 callback 必须额外回传本次 Product Gate 派发结果中的 `callback_provenance`，并填写与 registry/dispatch 一致的 `from_thread_id`；不得从 ledger、其它任务或历史 callback 复制 token。receipt 只有在签名有效且 ledger 存在对应 `evidence_receipt_issued` 时才算完成证据；历史 V2.1/V2.2 弱 dispatch 不能支撑新 completion。
...
- 缺少 Product Value Gate PASS 证据却实现 consumer/handoff/checkpoint/readiness/review surface。
- 为 UI consumer、报告镜像或 handoff 新建 capability 条目，或仅凭页面镜像推进 `integration_connected`。
