...
description: 当用户提到 GitHub 调研、开源库选型、找资源、找实现、github-research、opensrc、替换开源库、provider 候选、评估框架、图表库、领域数据源、指标/规则库、论文代码或需要比较仓库维护度/license/依赖复杂度时，使用这个 Skill 在 目标项目 文档体系内做开源研究；输出必须落到 docs/08-open-source-reference-pool.md、docs/09-research-roadmap.md、docs/capability-status.json 或泳道 workspace，不得直接复制外部代码进项目。
---

# 目标项目 开源研究执行器

目标：吸收 `github-research` 和开源分析类 skill 的方法，但把输出固定到 目标项目的研究/守门/能力状态文档。
...
当用户或主调度提到 GitHub 调研、开源库选型、找资源、找实现、github-research、opensrc、替换开源库、provider 候选、评估框架、图表库、领域数据源、指标/规则库、论文代码或需要比较仓库维护度/license/依赖复杂度时，优先使用本 Skill。
...
- 是否会引入真实 provider、付费、账号、远程写入、受监管操作/高风险自动化执行或高风险结论风险。
...
- 不要把开源 demo 的能力成熟度直接写成 目标项目 已接入。
